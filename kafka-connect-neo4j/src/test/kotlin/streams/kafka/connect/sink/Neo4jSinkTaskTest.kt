...
import org.neo4j.server.rest.domain.RelationshipExpanderBuilderTest.db
...
class Neo4jSinkTaskTest {
    ...
    @Test
    fun `should insert data into Neo4j with JSONParseException`() {
        val topic = "neotopic"
        val props = mutableMapOf<String, String>()
        props[Neo4jSinkConnectorConfig.ENCRYPTION_ENABLED] = false.toString()
        props[Neo4jSinkConnectorConfig.SERVER_URI] = db.boltURI().toString()
        ...
        val badData = """{"name": "San Mateo (CA)", latitude: 1.1, "longitude": -100.1}"""
        val data = """{"name": "San Mateo (CA)", "latitude": 1.1, "longitude": -100.1}"""

        val task = Neo4jSinkTask()
        task.initialize(mock(SinkTaskContext::class.java))
        task.start(props)
        task.put(listOf(SinkRecord(topic, 1, null, null, PERSON_SCHEMA, badData, 42)))
        task.put(listOf(SinkRecord(topic, 1, null, null, PERSON_SCHEMA, data, 43)))
        db.graph().beginTx().use {
            val node: Node? = db.graph().findNode(Label.label("Place"), "name", "San Mateo (CA)")
            assertNotNull(node)
        }
